...
- Create topic with 3 partitions because the topic `expedia` has 3 partitions. Name the new topic: `expedia_ext`:
...
1. Go into git `root` folder
2. Modify the file `/src/main.py` if requiried. (Verify the Kafka topic names)
3. ⚠️ To build the Docker image, choose the correct command based on your CPU architecture: 

    <details>
    <summary><code>Linux</code>, <code>Windows</code>, <code>&lt;Intel-based macOS&gt;</code> (<i>click to expand</i>)</summary>
    
    ```bash
    docker build -t <ACR_NAME>/stream-app:latest .
    ```
    ...
    ```bash
    docker build --platform linux/amd64 -t <ACR_NAME>/stream-app:latest .
    ```
...
```bash
docker push <ACR_NAME>/stream-app:latest
```

5. Deploy Stream Application into AKS

- modify the file `stream-app.yaml` (change the placeholder):
...
```yaml
    ...
    15   ⦙ ⦙ containers:
    16   ⦙ ⦙ ⦙ - name: kstream-app
    17   ⦙ ⦙ ⦙ ⦙ image: <ACR_NAME>/stream-app:latest
```

- deploy stream application by command:

```bash
kubectl apply -f stream-app.yaml
```

## 17. Verify the messages in Kafka

- Browse to Control Center: [http://localhost:9021](http://localhost:9021)
- Go into Cluster => Topics
- Choose topic name `expedia_ext`
- In the `messages` tab you should able to see messages

## 18.Visualize Data from Kafka Topic `expedia_ext` with KSQL

To work with KSQL, follow these steps:

1. Start by running the following command to access the KSQL command prompt:

```bash
kubectl exec -it ksqldb-0 -- ksql
```

You will be redirected in KSQL command promt.

2. Create a stream from `expedia_ext` topic:
...
If successful, you should see the message: `Stream created`.

3. Create table from stream as select query:
...
If successful, you should see the message: `Created query with ID ...`

4. Retrieve the total number of hotels (`hotel_id`) and the number of distinct hotels per category:

```sql
    SELECT * FROM hotels_count EMIT CHANGES;
```

This will return a table with results.

5. To visualize the data in the UI:

- Open Control Center [http://localhost:9021](http://localhost:9021)
- Navigate to  `Cluster` => `ksqlDB` => open the tab `Flow`

## 19. Destroy Infrastructure (Required Step)
